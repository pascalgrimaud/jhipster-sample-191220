...
export class BankAccountComponentsPage {
  ...
  deleteButtons = element.all(by.css('my-prefix-bank-account-my-suffix div table .btn-danger'));
  title = element.all(by.css('my-prefix-bank-account-my-suffix div h2#page-heading span')).first();
...
export class BankAccountUpdatePage {
  pageTitle = element(by.id('my-prefix-bank-account-my-suffix-heading'));
  ...
  meanQueueDurationInput = element(by.id('field_meanQueueDuration'));
  ...
  async setGuidInput(guid: string): Promise<void> {
    await this.guidInput.sendKeys(guid);
  }

  async getGuidInput(): Promise<string> {
    return await this.guidInput.getAttribute('value');
  }
  ...
  async setMeanQueueDurationInput(meanQueueDuration: string): Promise<void> {
    await this.meanQueueDurationInput.sendKeys(meanQueueDuration);
  }

  async getMeanQueueDurationInput(): Promise<string> {
    return await this.meanQueueDurationInput.getAttribute('value');
  }
...
export class BankAccountDeleteDialog {
  private dialogTitle = element(by.id('my-prefix-delete-bankAccount-heading'));
