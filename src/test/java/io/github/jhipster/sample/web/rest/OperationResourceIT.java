...
public class OperationResourceIT {
     ...
     */
    public static OperationEntity createEntity(EntityManager em) {
     ...
     * if they test an entity which requires the current entity.
     */
    public static OperationEntity createUpdatedEntity(EntityManager em) {
        OperationEntity operationEntity = new OperationEntity()
            .date(UPDATED_DATE)
    ...
    public void initTest() {
        operationEntity = createEntity(em);
    }
    ...
    public void createOperation() throws Exception {
        ...
        restOperationMockMvc.perform(post("/api/operations")
            ...
            .content(TestUtil.convertObjectToJsonBytes(operationEntity)))
            .andExpect(status().isCreated());

        // Validate the Operation in the database
        List<OperationEntity> operationList = operationRepository.findAll();
        assertThat(operationList).hasSize(databaseSizeBeforeCreate + 1);
        OperationEntity testOperation = operationList.get(operationList.size() - 1);
    ...
    public void checkAmountIsRequired() throws Exception {
        int databaseSizeBeforeTest = operationRepository.findAll().size();
        // set the field null
        operationEntity.setAmount(null);
    ...
    public void getAllOperations() throws Exception {
        ...
        restOperationMockMvc.perform(get("/api/operations?sort=id,desc"))
            ...
            .andExpect(jsonPath("$.[*].id").value(hasItem(operationEntity.getId().intValue())))
    ...
    public void getOperation() throws Exception {
        ...
        // Get the operation
        restOperationMockMvc.perform(get("/api/operations/{id}", operationEntity.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.id").value(operationEntity.getId().intValue()))
    ...
    public void updateOperation() throws Exception {
        ...
        OperationEntity updatedOperationEntity = operationRepository.findById(operationEntity.getId()).get();
        // Disconnect from session so that the updates on updatedOperationEntity are not directly saved in db
        em.detach(updatedOperationEntity);
        updatedOperationEntity
            .date(UPDATED_DATE)
            .description(UPDATED_DESCRIPTION)
            .amount(UPDATED_AMOUNT);
        ...
        restOperationMockMvc.perform(put("/api/operations")
            .contentType(TestUtil.APPLICATION_JSON_UTF8)
            .content(TestUtil.convertObjectToJsonBytes(updatedOperationEntity)))
            .andExpect(status().isOk());
    ...
    public void deleteOperation() throws Exception {
        ...
        restOperationMockMvc.perform(delete("/api/operations/{id}", operationEntity.getId())
